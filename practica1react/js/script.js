...
const obtenerVeredicto = (nota) => {
    if (nota >= 9) {
        ...
        return "Tremendo ganador, vamos a felicitarte";
    }
     else if (nota >= 5) {
...
boton.addEventListener('click', () => {
    ...
    let nombreJuego = inputJuego.value.trim();
    let nota = Number(inputCalificacion.value);

    // Validación básica por si dejan los campos vacíos o la nota no es un número
    if (nombreJuego === "" || isNaN(nota)) {
